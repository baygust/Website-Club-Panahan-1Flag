	...
	<?php
	if(isset($_POST['ubah'])){
		header("Location: akunPesertaUbah.php");
	}
	if(isset($_POST['password'])){
		header("Location: akunPesertaUbahPass.php");
	}
	if(isset($_POST['jadwal'])){
		header("Location: akunPesertaJadwal.php");
	}
	?>
	<body>
	<!-- awal Bagian navbar -->
		...
		<label><h3><b>Data Diri Anda (<?php echo $_SESSION['username']?>)</b></h3></label>
			...
			<form action="" method="post">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="nama" value="<?php echo $hasil_peserta['nama']; ?>" autocomplete="off" readonly>
				</div>
				<div class="form-group">
					<label>Nomor Telepon Anda : </label>
					<input type="number" class="form-control" name="no_telp" value="<?php echo $hasil_peserta['no_telp']; ?>" autocomplete="off" readonly>
				</div>
				<div class="form-group">
					<label>Email Anda : </label>
					<input type="email" class="form-control" name="email" value="<?php echo $hasil_peserta['email']; ?>" autocomplete="off" readonly>
				</div>
				<div class="form-group">
					<label>Alamat Anda : </label>
					<textarea class="form-control" name="alamat" rows="3" autocomplete="off" readonly><?php echo $hasil_peserta['alamat']; ?></textarea>
				</div>
				<div class="form-group">
					<button type="submit" class="btn btn-dark" name="ubah" >Ubah Data Diri</button>
					<button type="submit" class="btn btn-dark" name="password" >Ubah Password</button>
					<button type="submit" class="btn btn-dark" name="jadwal" >Jadwal Kursus Saya</button>
				</div>
			</form>
			</div>
		</div>
