...
<?php
	...
	$querytabel = "select * from pengajar where kode_pengajar='$_REQUEST[kode_pengajar]'";
...
if(isset($_POST['ya'])){
	...
	if($data_pengajar){
		...
		$query_reset_pengajar= "update login set password = '12345' where username='$username'";
		$reset_pengajar = mysqli_query($koneksi, $query_reset_pengajar);
		if($reset_pengajar){
			echo "<script> alert('Data berhasil di perbaharui menjadi 12345'); window.location.href='datapengajar.php'; </script>";
		}else{
...
<div class="container">
  ...
  <table class="table table-bordered" id="konten">
	...
	<center>
	<label>Yakin Data ini di reset password ?</label>
